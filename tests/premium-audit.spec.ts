...
test('featured views keep the chosen photographs and copy together after rapid input',async({page})=>{
  ...
  await expect(page.locator('[data-featured-title]')).toHaveText('Room to play. Places to gather.');
  ...
  await expect(page.locator('[data-featured-caption]')).toHaveText('Planting along the pedestrian frontage.');
